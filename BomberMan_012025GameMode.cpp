...
void ABomberMan_012025GameMode::BeginPlay()
{
	...
	// Asignamos los arrays de bloques y espacios vacíos desde la fachada
	// Esto es para que el GameMode tenga acceso a los bloques y espacios vacíos generados por el laberinto
	...
	SpawnPersonaje();
	// Crear trampa simple
	ATrampaSimple* Trampa1 = GetWorld()->SpawnActor<ATrampaSimple>(ATrampaSimple::StaticClass(), FVector(100, 0, 0), FRotator::ZeroRotator);
	ATrampaSimple* Trampa2 = GetWorld()->SpawnActor<ATrampaSimple>(ATrampaSimple::StaticClass(), FVector(200, 0, 0), FRotator::ZeroRotator);

	// Crear trampa compuesta
	ATrampaCompuesta* TrampaGrupo = GetWorld()->SpawnActor<ATrampaCompuesta>(ATrampaCompuesta::StaticClass(), FVector(0, 0, 0), FRotator::ZeroRotator);

	TrampaGrupo->AgregarTrampa(Trampa1);
	TrampaGrupo->AgregarTrampa(Trampa2);

	// Activar todo
	TrampaGrupo->ActivarTrampa(); // Activa Trampa1 y Trampa2
} 
